...
class UpdateTermEvaluationUseCaseImpl(UpdateTermEvaluationUseCase):
    ...
    async def __call__(self, args: Tuple[Tuple[HTTPAuthorizationCredentials, int], SaveTermEvaluationResource]) -> TermEvaluationResource:
        ...
        existing_term_evaluation.client_likes_term_interpretation = data.client_likes_term_interpretation
        existing_term_evaluation.client_likes_consumer_protection_law_matching = data.client_likes_consumer_protection_law_matching

        try:
            self.term_evaluation_repository.create(existing_term_evaluation)
            self.unit_of_work.commit()
        except Exception as _e:
            self.unit_of_work.rollback()
            raise CreateTermEvaluationError()

        updated_term_evaluation = self.term_evaluation_repository.find_by_term_id_and_profile_id(term_id, existing_profile.id)

        return TermEvaluationResource.from_entity(updated_term_evaluation)